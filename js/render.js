...
(function () {
  ...
  function dashboardView(state) {
    ...
    return (
      '<section class="section container dashboard">' +
        ...
        '<h2 class="subhead">Your bookings</h2>' + myBookingsTable(state) +
  ...
  // The renter's own booking requests + the owner's response.
  function myBookingsTable(state) {
    var list = state.outgoingBookings || [];
    if (!list.length) {
      return emptyState("No bookings yet", "Browse trees and request one — the owner's response shows up here.", "#/browse", "Browse trees");
    }
    var note = {
      pending: "Waiting for the owner to confirm.",
      accepted: "Confirmed! Arrange the harvest and payment directly with the owner.",
    ...
    var rows = list
      .map(function (b) {
        ...
        var canCancel = status === "pending" || status === "accepted";
        return (
          '<div class="data-row">' +
            "<div><strong>" + esc(title) + "</strong><br />" +
              '<span class="muted">' + esc(bookingWindowLabel(b)) + " &middot; " + esc(note[status] || "") + "</span></div>" +
            '<span class="status status--' + esc(status) + '">' + esc(cfg.labelFor(cfg.bookingStatuses, status)) + "</span>" +
            '<div class="data-row__actions">' +
              (canCancel
                ? '<button class="btn btn-sm btn-ghost" type="button" data-action="booking-status" data-id="' + esc(b.id) + '" data-status="cancelled">Cancel</button>'
                : "") +
            "</div>" +
          "</div>"
        );

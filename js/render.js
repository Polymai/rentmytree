...
(function () {
  ...
  function homeView(state) {
    ...
    return (
      ...
      '<section class="section container faq">' +
        ...
        faqItem("How much does it cost to post a tree?", "Listing a tree is free, and accepting or declining requests is free. You only pay a small fee (" + acceptFeeLabel(cfg.detectCurrency()) + ") once per season to unlock the messages on a listing — one fee lets you read and reply to everyone interested that season. Owners keep the harvest price they set.") +
        faqItem("How do payments work?", "Listing is free. When people request your tree, you pay one small unlock fee per season to open the messages and reply to all of them, then arrange the harvest and payment directly with the renter.") +
  ...
  function listingCard(state, listing) {
    ...
    return (
      '<article class="card">' +
        '<a class="card__link" href="#/listings/' + esc(listing.id) + '" data-nav aria-label="View ' + esc(listing.title || treeLabel(listing.tree_type) + " tree") + '">' + media + "</a>" +
        ...
        '<div class="detail__grid">' +
          ...
          '<aside class="detail__aside">' +
            '<div class="booking-card">' +
              ...
              "</button>" +
              (owner ? "" : '<p class="muted booking-card__hint">Questions? Send them with your booking request above — the grower replies in the booking chat.</p>') +
            "</div>" +
          "</aside>" +
        "</div>" +
      "</section>"
  ...
  function routeNotice(state) {
    ...
    if (query.unlock === "success") {
      return (
        '<div class="route-notice route-notice--success" role="status">' +
          "<strong>Messages unlocked.</strong>" +
          "<span>You can now read and reply to every request on this listing for the season. Open a booking to start chatting.</span>" +
        "</div>"
      );
    }
    if (query.unlock === "cancelled") {
      return (
        '<div class="route-notice" role="status">' +
          "<strong>Unlock cancelled.</strong>" +
          "<span>The messages stay locked until you unlock the season from a booking.</span>" +
        "</div>"
      );
    }
    return "";
  ...
  /* ---------------------------- dashboard ----------------------------- */
  /* ------------------- dashboard family (hub + tabs) ------------------ */
  function dashboardTabs(active) {
    var tabs = [
      ["dashboard", "#/dashboard", "Overview"],
      ["bookings", "#/bookings", "Bookings"],
      ["my-listings", "#/my-listings", "Listings"],
      ["payments", "#/payments", "Payments"],
    ];
    return (
      '<nav class="dash-tabs" aria-label="Dashboard sections">' +
        tabs.map(function (t) {
          return '<a class="dash-tab' + (t[0] === active ? " is-active" : "") + '" href="' + t[1] + '" data-nav>' + esc(t[2]) + "</a>";
        }).join("") +
      "</nav>"
    );
  }

  function dashboardHead(title, sub) {
    return (
      '<div class="section-head">' +
        "<div><h1>" + esc(title) + "</h1><p class=\"muted\">" + esc(sub) + "</p></div>" +
        '<a class="btn btn-primary" href="#/listings/new" data-nav>List a tree</a>' +
      "</div>"
    );
  }

  function statTile(href, num, label, badge) {
    var b = badge ? ' <span class="stat-tile__badge">' + esc(String(badge)) + "</span>" : "";
    return (
      '<a class="stat-tile" href="' + href + '" data-nav>' +
        '<span class="stat-tile__num">' + esc(String(num)) + "</span>" +
        '<span class="stat-tile__label">' + esc(label) + b + "</span>" +
      "</a>"
    );
  }

  // All bookings the user is party to, newest first, tagged with their role.
  function combineBookings(state) {
    var out = [];
    (state.incomingBookings || []).forEach(function (b) { out.push({ b: b, role: "owner" }); });
    (state.outgoingBookings || []).forEach(function (b) { out.push({ b: b, role: "renter" }); });
    out.sort(function (a, b) { return new Date(b.b.created_at || 0) - new Date(a.b.created_at || 0); });
    return out;
  }

  function bookingRow(state, b, role) {
    var title = b.listings ? b.listings.title : "Listing";
    var status = b.status || "pending";
    var roleLabel = role === "owner" ? "Request on your tree" : "Your request";
    var attention = role === "owner" && status === "pending";
    return (
      '<a class="booking-item' + (attention ? " booking-item--attention" : "") + '" href="#/bookings/' + esc(b.id) + '" data-nav>' +
        '<span class="booking-item__body">' +
          '<strong class="booking-item__title">' + esc(title) + "</strong>" +
          '<span class="booking-item__meta">' + esc(roleLabel) + " &middot; " + esc(bookingWindowLabel(b)) + "</span>" +
        "</span>" +
        '<span class="status status--sm status--' + esc(status) + '">' + esc(cfg.labelFor(cfg.bookingStatuses, status)) + "</span>" +
        '<span class="booking-item__chev" aria-hidden="true">&rarr;</span>' +
      "</a>"
    );
  }

  // Status groups for the Bookings sub-menu.
  var BOOKING_GROUPS = {
    active: { label: "Active", match: ["pending", "accepted"] },
    completed: { label: "Completed", match: ["completed"] },
    cancelled: { label: "Cancelled", match: ["cancelled", "declined"] },
    all: { label: "All", match: null },
  };
  function bookingGroupMatch(group, status) {
    var g = BOOKING_GROUPS[group] || BOOKING_GROUPS.active;
    return !g.match || g.match.indexOf(status) >= 0;
  }
  function bookingSubnav(active, incoming, outgoing) {
    var all = (incoming || []).concat(outgoing || []);
    return (
      '<nav class="chip-nav" aria-label="Filter bookings by status">' +
        Object.keys(BOOKING_GROUPS).map(function (key) {
          var n = key === "all" ? all.length : all.filter(function (b) { return bookingGroupMatch(key, b.status || "pending"); }).length;
          return '<a class="chip-nav__item' + (key === active ? " is-active" : "") + '" href="#/bookings?status=' + key + '" data-nav>' +
            esc(BOOKING_GROUPS[key].label) + ' <span class="chip-nav__count">' + n + "</span></a>";
        }).join("") +
      "</nav>"
    );
  }
  ...
  function dashboardView(state) {
    if (!state.user) return authGate(state, "Sign in to your dashboard", "signin");
    var pending = (state.incomingBookings || []).filter(function (b) { return b.status === "pending"; }).length;
    var paidUnlocks = (state.myUnlocks || []).filter(function (u) { return u.status === "paid"; }).length;
    var totalBookings = (state.incomingBookings || []).length + (state.outgoingBookings || []).length;
    var recent = combineBookings(state).slice(0, 5);
    var recentBlock = recent.length
      ? '<div class="data-list">' + recent.map(function (x) { return bookingRow(state, x.b, x.role); }).join("") + "</div>"
      : emptyState("No bookings yet", "Requests you make or receive show up here.", "#/browse", "Browse trees");

    return (
      '<section class="section container dashboard">' +
        dashboardHead("Your dashboard", "Bookings, listings and payments — all in one place.") +
        dashboardTabs("dashboard") +
        routeNotice(state) +
        '<div class="stat-row">' +
          statTile("#/bookings", totalBookings, "Bookings", pending || 0) +
          statTile("#/my-listings", (state.myListings || []).length, "Listings") +
          statTile("#/payments", paidUnlocks, "Unlocked") +
        "</div>" +
        '<h2 class="subhead">Recent bookings</h2>' + recentBlock +
      "</section>"
    );
  }

  function bookingsView(state) {
    if (!state.user) return authGate(state, "Sign in to see your bookings", "signin");
    var incomingAll = state.incomingBookings || [];
    var outgoingAll = state.outgoingBookings || [];
    var q = (state.route && state.route.params && state.route.params.query) || {};
    var group = BOOKING_GROUPS[q.status] ? q.status : "active";
    var byGroup = function (b) { return bookingGroupMatch(group, b.status || "pending"); };
    var incoming = incomingAll.filter(byGroup);
    var outgoing = outgoingAll.filter(byGroup);

    var incBlock = incoming.length
      ? '<div class="data-list data-list--compact">' + incoming.map(function (b) { return bookingRow(state, b, "owner"); }).join("") + "</div>"
      : '<p class="muted booking-empty">No ' + esc(BOOKING_GROUPS[group].label.toLowerCase()) + " requests on your trees.</p>";
    var outBlock = outgoing.length
      ? '<div class="data-list data-list--compact">' + outgoing.map(function (b) { return bookingRow(state, b, "renter"); }).join("") + "</div>"
      : '<p class="muted booking-empty">No ' + esc(BOOKING_GROUPS[group].label.toLowerCase()) + " requests from you.</p>";

    return (
      '<section class="section container dashboard">' +
        dashboardHead("Bookings", "Requests on your trees, and trees you've asked to rent.") +
        dashboardTabs("bookings") +
        routeNotice(state) +
        bookingSubnav(group, incomingAll, outgoingAll) +
        '<h2 class="subhead">Requests on your trees</h2>' + incBlock +
        '<h2 class="subhead">Your booking requests</h2>' + outBlock +
      "</section>"
    );
  }

  function myListingsView(state) {
    if (!state.user) return authGate(state, "Sign in to manage your listings", "signin");
    var block;
    if (state.myListingsLoading) block = loadingGrid();
    else if (!(state.myListings || []).length) block = emptyState("No listings yet", "List your first tree to start renting it out.", "#/listings/new", "List a tree");
    else block = '<div class="owner-grid">' + state.myListings.map(ownerListingRow.bind(null, state)).join("") + "</div>";
    return (
      '<section class="section container dashboard">' +
        dashboardHead("Your listings", "The trees you offer for rent.") +
        dashboardTabs("my-listings") +
        block +
      "</section>"
    );
  }
  ...
  function messagesView(state) {
    if (!state.user) return authGate(state, "Sign in to see your messages", "signin");
    return (
      '<section class="section container dashboard">' +
        dashboardHead("Messages", "Conversations with growers and renters.") +
        dashboardTabs("messages") +
        requestsList(state) +
      "</section>"
    );
  }
  ...
  /* ------------------------- booking detail --------------------------- */
  function bookingStatusFlow(status) {
    if (status === "declined" || status === "cancelled") {
      return '<div class="booking-flow booking-flow--ended"><span class="status status--' + esc(status) + '">' +
        esc(cfg.labelFor(cfg.bookingStatuses, status)) + "</span></div>";
    }
    var steps = ["pending", "accepted", "completed"];
    var labels = { pending: "Requested", accepted: "Accepted", completed: "Completed" };
    var idx = steps.indexOf(status);
    if (idx < 0) idx = 0;
    return (
      '<ol class="booking-flow">' +
        steps.map(function (s, i) {
          var cls = i < idx ? "is-done" : (i === idx ? "is-current" : "");
          return '<li class="booking-flow__step ' + cls + '">' + esc(labels[s]) + "</li>";
        }).join("") +
      "</ol>"
    );
  }

  function bookingStatusNote(status, isOwner) {
    if (status === "pending") return isOwner ? "This renter is waiting for your answer. Accept to confirm or decline — both are free. Unlock messages to read their note and reply." : "Waiting for the grower to confirm your request.";
    if (status === "accepted") return "Confirmed. Use the messages below to arrange the harvest and payment directly.";
    if (status === "completed") return "This booking is complete.";
    if (status === "declined") return isOwner ? "You declined this request." : "The grower declined this request.";
    if (status === "cancelled") return "This request was cancelled.";
    return "";
  }

  function bookingThreadBlock(state, b, thread) {
    var me = state.user && state.user.id;
    var l = b.listings || {};
    if (!thread) return '<p class="muted">No messages yet.</p>';
    var msgs = [];
    if (thread.message) msgs.push({ author: thread.sender_user_id, text: thread.message, at: thread.created_at });
    (thread.contact_replies || []).slice().sort(function (a, c) {
      return new Date(a.created_at || 0) - new Date(c.created_at || 0);
    }).forEach(function (r) { msgs.push({ author: r.author_user_id, text: r.message, at: r.created_at }); });
    var bubbles = msgs.map(function (m) {
      var mine = m.author === me;
      var who = mine ? "You" : (m.author === l.seller_user_id ? "Grower" : "Renter");
      return (
    ...
    var st = state.requestStatus["reply-" + thread.id] || {};
    var form =
      '<form class="reply-form" data-form="message-reply" data-request-id="' + esc(thread.id) + '">' +
        '<label class="sr-only" for="booking-msg">Message</label>' +
        '<textarea id="booking-msg" name="message" rows="2" required placeholder="Write a message…"></textarea>' +
        '<div class="reply-form__actions"><button class="btn btn-primary btn-sm" type="submit"' + (st.loading ? " disabled" : "") + ">Send</button></div>" +
        (st.error ? '<p class="form-status form-status--error">' + esc(st.error) + "</p>" : "") +
      "</form>";
    return '<div class="msg-list">' + (bubbles || '<p class="muted">No messages yet.</p>') + "</div>" + form;
  }

  function bookingDetailView(state) {
    if (!state.user) return authGate(state, "Sign in to view this booking", "signin");
    if (state.activeBookingLoading) return '<section class="section container"><div class="skeleton-detail">Loading booking…</div></section>';
    var b = state.activeBooking;
    if (!b) return '<section class="section container">' + emptyState("Booking not found", state.activeBookingError || "This booking may have been removed.", "#/bookings", "Back to bookings") + "</section>";
    var l = b.listings || {};
    var isOwner = l.seller_user_id === (state.user && state.user.id);
    var status = b.status || "pending";
    // The owner sees the thread only once this listing+season is unlocked; the
    // gated thread is simply absent from the data, so its presence == unlocked.
    var messagesLocked = isOwner && !state.activeBookingThread;
    var seasonYear = b.start_date ? String(b.start_date).slice(0, 4) : "";
    var hasPin = l.latitude != null && l.longitude != null && isFinite(l.latitude) && isFinite(l.longitude);
    var feeCode = l.listing_fee_currency ? l.listing_fee_currency.toUpperCase() : cfg.detectCurrency();
    var unlockFee = money(cfg.unlockFeeFor(feeCode).gross, feeCode, cfg.currencyDecimals(feeCode));
    var priceCode = feeCode;
    var priceLabel = (b.total_cents != null ? b.total_cents : l.price_cents) != null
      ? money((b.total_cents != null ? b.total_cents : l.price_cents), priceCode, cfg.currencyDecimals(priceCode)) + " " + esc(periodLabel(l.rental_period))
      : "";

    var actions = "";
    if (isOwner && status === "pending") {
      if (messagesLocked) {
        // Can't accept blind — unlock (read the request) first. Declining is free.
        actions =
          '<button class="btn btn-primary btn-block" type="button" data-action="unlock-listing" data-listing="' + esc(l.id) + '" data-season="' + esc(seasonYear) + '">Unlock ' + esc(seasonYear) + " (" + esc(unlockFee) + ") to accept</button>" +
          '<button class="btn btn-ghost btn-block" type="button" data-action="booking-status" data-id="' + esc(b.id) + '" data-status="declined">Decline</button>' +
          '<p class="booking-side__hint muted">Unlock to read the request and reply before you confirm.</p>';
      } else {
        actions =
          '<button class="btn btn-primary btn-block" type="button" data-action="booking-status" data-id="' + esc(b.id) + '" data-status="accepted">Accept booking</button>' +
          '<button class="btn btn-ghost btn-block" type="button" data-action="booking-status" data-id="' + esc(b.id) + '" data-status="declined">Decline</button>';
      }
    } else if (isOwner && status === "accepted") {
      actions = '<button class="btn btn-primary btn-block" type="button" data-action="booking-status" data-id="' + esc(b.id) + '" data-status="completed">Mark complete</button>';
    } else if (!isOwner && (status === "pending" || status === "accepted")) {
      actions = '<button class="btn btn-ghost btn-block" type="button" data-action="booking-status" data-id="' + esc(b.id) + '" data-status="cancelled">Cancel request</button>';
    }

    function fact(label, val) {
      return '<div class="booking-fact"><span class="booking-fact__k">' + esc(label) + '</span><span class="booking-fact__v">' + esc(val) + "</span></div>";
    }

    return (
      '<section class="section container booking-detail">' +
        '<a class="back-link" href="#/bookings" data-nav>&larr; Back to bookings</a>' +
        routeNotice(state) +
        "<h1>" + esc(l.title || "Booking") + "</h1>" +
        '<div class="booking-detail__grid">' +
          '<div class="booking-detail__main">' +
            bookingStatusFlow(status) +
            '<p class="muted booking-detail__note">' + esc(bookingStatusNote(status, isOwner)) + "</p>" +
            '<div class="booking-facts">' +
              fact("When", bookingWindowLabel(b)) +
              (l.tree_type ? fact("Type", treeLabel(l.tree_type)) : "") +
              (l.city ? fact("Location", l.city) : "") +
            "</div>" +
            (priceLabel ? '<p class="booking-detail__price">Harvest price: ' + priceLabel + "</p>" : "") +
            (hasPin ? '<div id="booking-map" class="map map--detail" role="application" aria-label="Tree location"></div>' : (l.city ? "" : '<p class="muted">Exact location shared after confirmation.</p>')) +
            '<h2 class="detail__subhead">Messages</h2>' +
            (messagesLocked
              ? '<div class="msg-lock">' +
                  '<span class="msg-lock__icon" aria-hidden="true">🔒</span>' +
                  "<p><strong>People want to rent this tree.</strong> Unlock messages for the " + esc(seasonYear) + " season to read every request on this listing and reply — one fee covers all of them for the season.</p>" +
                  '<button class="btn btn-primary btn-sm" type="button" data-action="unlock-listing" data-listing="' + esc(l.id) + '" data-season="' + esc(seasonYear) + '">Unlock messages for ' + esc(seasonYear) + " (" + esc(unlockFee) + ")</button>" +
                "</div>"
              : bookingThreadBlock(state, b, state.activeBookingThread)) +
          "</div>" +
          '<aside class="booking-detail__side">' +
            '<div class="booking-side__card">' +
              '<span class="status status--' + esc(status) + '">' + esc(cfg.labelFor(cfg.bookingStatuses, status)) + "</span>" +
              (actions ? '<div class="booking-side__actions">' + actions + "</div>" : "") +
            "</div>" +
          "</aside>" +
        "</div>" +
      "</section>"
  ...
  function paymentsPanel(state) {
    ...
    // Message-unlock payments the owner has made (or started), per listing+season.
    var unlocks = (state.myUnlocks || []).filter(function (u) {
      return u.status && u.status !== "unpaid";
    });
    // Any legacy listings that actually paid a listing fee.
    var legacyPaid = (state.myListings || []).filter(function (l) {
      return l.listing_fee_status === "paid";
    });
    if (!unlocks.length && !legacyPaid.length) {
      return emptyState("No payments yet", "When you unlock a listing's messages for a season, the payment and receipt show up here.");
    }
    var rows = unlocks.map(unlockPaymentRow).join("") + legacyPaid.map(paymentRow).join("");
    return '<div class="payments-list">' + rows + "</div>";
  }

  function feeStatusLabel(v) {
  ...
  function unlockPaymentRow(u) {
    var status = u.status || "unpaid";
    var code = (u.currency || cfg.detectCurrency()).toUpperCase();
    var amountLabel = u.amount_cents
      ? money(u.amount_cents, code, cfg.currencyDecimals(code)) + " incl. VAT"
      : "";
    var title = (u.listings ? u.listings.title : "Listing") + (u.season_year ? " · " + u.season_year : "");
    var paidAt = u.paid_at ? new Date(u.paid_at).toLocaleDateString() : "";
    var meta = paidAt ? "Paid " + paidAt : "In progress";
    var actions = "";
    if (status === "paid" && u.receipt_url) {
      actions = '<a class="btn btn-sm btn-outline" href="' + esc(u.receipt_url) + '" target="_blank" rel="noopener">View receipt</a>';
    }
    return (
      '<div class="payment-row">' +
        '<div class="payment-row__main">' +
          "<strong>" + esc(title) + "</strong>" +
          '<span class="muted">' + esc(amountLabel) + " message unlock &middot; " + esc(meta) + "</span>" +
        "</div>" +
        '<span class="payment-pill payment-pill--' + esc(status) + '">' + esc(feeStatusLabel(status)) + "</span>" +
        '<div class="payment-row__actions">' + actions + "</div>" +
      "</div>"
    );
  }
  ...
  function paymentsView(state) {
    if (!state.user) return authGate(state, "Sign in to view payments", "signin");
    return (
      '<section class="section container dashboard payments-page">' +
        dashboardHead("Payments", "Your booking acceptance fees and receipts.") +
        dashboardTabs("payments") +
        routeNotice(state) +
        paymentsPanel(state) +
      "</section>"
    );
  ...
  function editorView(state) {
    if (!state.user) return authGate(state, "Sign in to list a tree", "signin");
    ...
    return (
      '<section class="section container editor">' +
        ...
        '<div class="cost-banner" role="note">' +
          ...
          '<span>Accepting requests is free. You only pay a small fee (<span class="js-accept-fee">' + esc(acceptFeeLabel(curCode)) + "</span>) once per season to unlock this listing's messages. The price below is the harvest price you set.</span>" +
        "</div>" +
  ...
  function favoritesView(state) {
    if (!state.user) return authGate(state, "Sign in to see your favorites", "signin");
    var content;
  ...
  function settingsView(state) {
    if (!state.user) return authGate(state, "Sign in to manage your account", "signin");
    var p = state.profile || {};
  ...
  /* ------------------------ shared sub-views -------------------------- */
  // Protected routes: while the session is still being restored, show a quiet
  // "restoring" state instead of the sign-in gate — avoids a false "signed out"
  // flash on refresh / return from Stripe.
  function authGate(state, title, mode) {
    if (!state.authReady) {
      return (
        '<section class="section container">' +
          '<div class="auth-gate"><p class="muted">Restoring your session…</p></div>' +
        "</section>"
      );
    }
    return authGateView(title, mode);
  }
  ...
  function routeView(state) {
    switch (state.route.name) {
      ...
      case "bookings": return bookingsView(state);
      case "booking": return bookingDetailView(state);
      case "my-listings": return myListingsView(state);
  ...
  var maps = { browse: null, detail: null, editor: null, booking: null };
  ...
  function afterRender(state) {
    ...
    if (state.route.name !== "booking") destroyMap("booking");
    ...
    if (state.route.name === "booking") mountBookingMap(state);
  ...
  function mountBookingMap(state) {

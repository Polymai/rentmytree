...
(function () {
  ...
  var AppConfig = {
    ...
    pricing: {
      ...
      // Fixed fee the owner pays once per listing per harvest year (season) to
      // unlock that listing's booking message threads. GROSS incl VAT (minor
      // units). Keep in sync with the UNLOCK_FEE table in the Edge function.
      unlockFee: {
  ...
  // Unlock fee for a currency: the stored amount is GROSS (VAT included), so we
  // back out the net + VAT parts for display/receipts.
  AppConfig.unlockFeeFor = function (code) {
    code = String(code || AppConfig.detectCurrency()).toUpperCase();
    var table = AppConfig.pricing.unlockFee;
  ...
  AppConfig.acceptanceFeeFor = AppConfig.unlockFeeFor;

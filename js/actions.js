...
(function () {
  ...
  function requireAuth() {
    ...
    // Only prompt sign-in once the initial session restore has actually run.
    // Before that, a refresh / return-from-Stripe would flash the login popup
    // even though the user is still signed in (session just not restored yet).
    if (s.authReady) {
  ...
  function findContactRequest(id) {
    ...
    var thread = State.getState().activeBookingThread;
    if (thread && thread.id === id) return thread;
    return null;
  }
  ...
  var Loaders = {
    ...
    dashboard: function () {
      ...
      return Promise.all([
        ...
        Api.listMyUnlocks(uid).catch(function () { return []; }),
      ]).then(function (res) {
        State.setState({
          myListings: res[0] || [],
          incomingBookings: res[1] || [],
          outgoingBookings: res[2] || [],
          myUnlocks: res[3] || [],
          incomingRequests: [],
    ...
    bookingDetail: function (id) {
      ...
      State.setState({ activeBookingLoading: true, activeBookingError: "" });
      return Promise.all([
        Api.getBooking(id),
        Api.getBookingThread(id).catch(function () { return []; }),
      ])
        .then(function (res) {
          var booking = res[0];
          var threads = res[1] || [];
          var thread = threads.length ? threads[0] : null;
          // Renter-side: make sure a conversation thread exists (sender=renter so
          // existing RLS covers both parties). Owner never needs to create it.
          var me = State.getState().user;
          if (booking && !thread && me && booking.renter_user_id === me.id) {
            return Api.createContactRequest({
              listing_id: booking.listing_id,
              booking_id: booking.id,
              sender_user_id: me.id,
              message: booking.message || "Booking conversation",
              email: (me && me.email) || null,
              status: "read",
            })
              .then(function () { return Api.getBookingThread(id); })
              .then(function (t2) {
                State.setState({ activeBooking: booking, activeBookingThread: (t2 || [])[0] || null, activeBookingLoading: false });
              })
              .catch(function () {
                State.setState({ activeBooking: booking, activeBookingThread: null, activeBookingLoading: false });
              });
          }
          State.setState({ activeBooking: booking || null, activeBookingThread: thread, activeBookingLoading: false });
        })
        .catch(function (err) {
          State.setState({ activeBookingLoading: false, activeBookingError: friendly(err) });
        });
    },
  ...
  function loadRoute(route) {
    switch (route.name) {
      ...
      case "bookings": Loaders.dashboard(); break;
      case "booking": Loaders.bookingDetail(route.params.id); break;
      case "my-listings": Loaders.dashboard(); break;
  ...
  // Reload the data behind whichever dashboard-family view is showing.
  function refreshAfterAction() {
    var r = State.getState().route;
    if (r.name === "booking" && r.params && r.params.id) Loaders.bookingDetail(r.params.id);
    else Loaders.dashboard();
  }
  ...
  var clickHandlers = {
    ...
    "booking-status": function (el) {
      // Accept / decline / complete / cancel are all free status changes now;
      // payment is the separate per-listing-per-season message unlock.
      var id = el.getAttribute("data-id");
      var status = el.getAttribute("data-status");
      Api.updateBookingStatus(id, status)
        .then(function () { toast("Booking updated.", "info"); refreshAfterAction(); })
        .catch(function (err) { toast(friendly(err), "error"); });
    },
    "unlock-listing": function (el) {
      var listingId = el.getAttribute("data-listing");
      var seasonYear = el.getAttribute("data-season");
      if (!listingId || !seasonYear) return;
      showLoader("Opening secure checkout…");
      Api.createListingUnlockCheckoutSession(listingId, Number(seasonYear))
        .then(function (checkout) {
          if (checkout && checkout.alreadyPaid) { hideLoader(); refreshAfterAction(); return; }
    ...
    booking: function (form) {
      ...
      ensureProfileReady().then(function (profile) {
        ...
        return Api.createBooking({
          ...
          // Only the neutral window summary lives on the booking (the owner can
          // read this freely). The renter's freeform note goes in the gated
          // thread so it's only readable after the owner pays to accept.
          message: (windowInfo && windowInfo.messagePrefix) || null,
          total_cents: listing ? listing.price_cents : null,
          status: "pending",
        })
          .then(function (booking) {
            // Seed the booking's conversation thread with the full note. The
            // renter is the sender; RLS lets the owner read/reply only once the
            // acceptance fee is paid.
            return Api.createContactRequest({
              listing_id: id,
              booking_id: booking.id,

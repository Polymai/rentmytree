...
(function () {
  ...
  var AppApi = {
    ...
    createListingUnlockCheckoutSession: function (listingId, seasonYear) {
      return authHeader().then(function (authorization) {
        var href = window.location.origin + window.location.pathname;
        return fetch(functionUrl("app702-rentmytree-api"), {
          method: "POST",
          ...
          body: JSON.stringify({
            action: "create_listing_unlock_checkout_session",
            listingId: listingId,
            seasonYear: seasonYear,
            successUrl: href + "#/bookings?unlock=success&listing=" + encodeURIComponent(listingId),
            cancelUrl: href + "#/bookings?unlock=cancelled&listing=" + encodeURIComponent(listingId),
          }),
        }).then(parseFunctionResponse);
      });
    },

    // The owner's message-unlock payments (per listing + season), for Payments.
    listMyUnlocks: function (sellerUserId) {
      return db()
        .from("listing_season_unlocks")
    ...
    getBooking: function (id) {
      return db()
        .from("bookings")
        .select("*, listings(id, title, city, address, latitude, longitude, seller_user_id, rental_period, tree_type, price_cents, listing_fee_currency)")
        .eq("id", id)
        .maybeSingle()
        .then(unwrap);
    },

    // The conversation thread tied to a booking (one contact_request + replies).
    getBookingThread: function (bookingId) {
      return db()
        .from("contact_requests")
        .select("*, listings(id, title, seller_user_id), contact_replies(id, request_id, author_user_id, message, created_at)")
